...
class Database(object):
    def __init__(self):
        self.client = MongoClient(config['db']['url'])  # set db url
        self.db = self.client[config['db']['name']]  # set db name
        
        #create collection and index on product_id on inititalization
        db = self.client.get_default_database()
        my_collection = self.db['products']
        index_name = 'product_id_idx'
        my_collection.create_index('product_id', name=index_name, unique=True)
    ...
    def find_by_id(self, product_id, collection_name):
        found = self.db[collection_name].find_one({"product_id" : product_id},{"_id": False})
        
        if found is None:
            return "product doesnt exists"
        
        if "product_id" in found:
            found["product_id"] = str(found["product_id"])
